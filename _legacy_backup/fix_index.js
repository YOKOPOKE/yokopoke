                ...
                <div class="bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
                    ...
                    <div class="flex justify-between items-start">
                        ...
                        <div class="text-right">
                             <span class="block font-bold text-yoko-primary text-xl" id="summary-price">$0.00</span>
                        </div>
                    </div>
                </div>
...
try {
    ...
    // Find the marker for the checkout view start
    // We look for the ID because it's the most reliable unique identifier for the start of the bad block
    // OR we can look for the comment if we want to include it.
    // Let's use the ID and backtrack slightly if needed, or just replace from the ID.
    // The previous Python script logic was sound: find "checkout-view" ID.

    let startIndex = content.indexOf('id="checkout-view"');

    // Find the LAST style tag, which marks the end of our replacement zone (we want to keep the style tag)
    let endIndex = content.lastIndexOf('<style>');

    if (startIndex !== -1 && endIndex !== -1) {
        // Adjust Start Index to include the opening <div tag which contains the ID
        // The indexOf 'id="checkout-view"' points to inside the tag.
        // We need to find the '<div' before it.
        const divStart = content.lastIndexOf('<div', startIndex);

        // Also check if there is a comment "FULL SCREEN CHECKOUT VIEW" right before it logic
        // But simply replacing from <div id="checkout-view"... is safe enough.

        console.log(`Replace from index ${divStart} to ${endIndex}`);

        const topPart = content.substring(0, divStart);
        const bottomPart = content.substring(endIndex);

        const finalContent = topPart + newCheckoutHtml + '\n' + bottomPart;
